<script lang="ts">
	...
	data.dedications.forEach((element: any) => {
		totalHours += element.hours;
	});

	const isAdmin = data.user?.id_rol === 1 ? true : false;
</script>
		...
		<div class="flex flex-col sm:pl-4 items-center justify-center gap-x-14">
			<section class="flex items-center flex-col lg:flex-row justify-between w-full">
				<div class="w-full lg:w-1/3 pt-4 pb-8">
					...
					{:else}
						<p class="text-lg text-dark-text pt-2">
							No olvides verificar la jornada laboral de tus usuarios.
						</p>
						<div class="flex items-center justify-start pt-4">
							<a
								href="/u/horas"
			...
			{#if data?.dedications?.length > 0 && !isAdmin}
